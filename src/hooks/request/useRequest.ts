...
// 引入返回值类型
import type { HttpResponse } from '@/utils/http/index';
...
const useRequest = <ParamType, PromiseRequestType, DataType = HttpResponse<PromiseRequestType>>(
  ...
  if (option.throttle && option.debounce) {
    console.warn(
      '[HttpResponse warn]: useRequest的配置项中的throttle和debounce均为true，请选择一个，否则这样默认使用防抖'
    );
  }
